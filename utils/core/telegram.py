...
class Accounts:
    ...
    def get_available_accounts(sessions: list):
        ...
        if config.PROXY['USE_PROXY_FROM_FILE']:
            ...
            if not accounts_from_json:
                raise ValueError("Нету аккаунтов в sessions/accounts.json")
    ...
    async def check_valid_accounts(self, accounts: list):
        ...
        logger.success(f"Валидные аккаунты: {len(valid_accounts)}; Невалидные: {len(invalid_accounts)}")
    ...
    async def get_accounts(self):
        ...
        if not available_accounts:
            raise ValueError("Нету валидных аккаунтов!")
        else:
            logger.success(f"Найдено активных аккаунтов: {len(available_accounts)}.")
    ...
    async def create_sessions(self):
        while True:
            ...
            if config.PROXY['USE_PROXY_FROM_FILE']:
                ...
                proxy = input("Вставте proxy в формате login:password@ip:port (нажмите Enter чтоб использовать без прокси): ")

            dict_proxy = self.parse_proxy(proxy) if proxy else None

            phone_number = (input("Введите номер телефона учетной записи: ")).replace(' ', '')
