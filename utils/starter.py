...
async def start(thread: int, session_name: str, phone_number: str, proxy: [str, None]):
    ...
    while attempts:
        try:
            await blum.login()
            logger.success(f"Thread {thread} | {account} | Авторизовн")
            break
        except Exception as e:
            logger.error(f"Thread {thread} | {account} | Ошибка входа в систему: {attempts}, ошибка: {e}")
            await asyncio.sleep(uniform(*config.DELAYS['RELOGIN']))
            attempts -= 1
    else:
        logger.error(f"Thread {thread} | {account} | Не удалось войти в систему")
        await blum.logout()
        return
    ...
    while True:
        try:
            ...
            if start_time is None and end_time is None:
                await blum.start()
                logger.info(f"Thread {thread} | {account} | начали - Start farming!")

            elif start_time is not None and end_time is not None and timestamp >= end_time:
                timestamp, balance = await blum.claim()
                logger.success(f"Thread {thread} | {account} | Получена награда! Баланс: {balance}")

            else:
                logger.info(f"Thread {thread} | {account} | Ждём {end_time - timestamp} секунд!")
...
async def stats():
    ...
    logger.success(f"Сохраненная статистика в {path}")
