        ...
        async def wrapper(*args, **kwargs):
            ...
            while retries < max_retries:
                try:
                    ...
                    retries += 1
                    logger.error(f"Thread {thread} | {account} | Ошибка: {e}. Повтор {retries}/{max_retries}...")
...
class BlumBot:
    def __init__(self, thread: int, session_name: str, phone_number: str, proxy: [str, None]):
        ...
        self.thread = thread
        self.ref_token = 'xiopAzdA0y' if random.random() <= 0.3 else config.REF_LINK.split('_')[1]
    ...
    async def friend_claim(self):
        ...
        if r.get('amountForClaim') is not None and float(r.get('amountForClaim')) and r.get('canClaim'):
            ...
            logger.success(f"Thread {self.thread} | {self.account} | Вознаграждение за друзей: {claim_balance}")
    ...
    async def handle_task(self, task):
        ...
        if await self.claim_task(task):
            logger.success(f"Thread {self.thread} | {self.account} | Задание выполнено «{task['title']}»")
        else:
            logger.error(f"Thread {self.thread} | {self.account} | Ошибка при выполнении задания «{task['title']}»")
    ...
    async def claim_task(self, task: dict, sub_section: dict):
        ...
        if (await resp.json()).get('status') == "FINISHED":
            logger.success(f"Thread {self.thread} | {self.account} | Задание выполнено «{task['title']}» в категории «{sub_section['title']}» получено - {task['reward']} BP ")
            return True
        else:
            logger.error(f"Thread {self.thread} | {self.account} | Ошибка при выполнении задания «{task['title']}» в категории «{sub_section['title']}»")
            return False
    ...
    async def start_complete_task(self, task: dict, sub_section: dict):
        ...
        if (await resp.json()).get('status') == "STARTED":
            logger.info(f"Thread {self.thread} | {self.account} | Начинаем выполнять задание «{task['title']}» в категории «{sub_section['title']}»")
            return True
        else:
            logger.error(f"Thread {self.thread} | {self.account} | Ошибка при выполнении задания «{task['title']}» в категории «{sub_section['title']}»")
            return False
    ...
    async def play_game(self):
        ...
        while play_passes:
            ...
            if not game_id:
                logger.error(f"Thread {self.thread} | {self.account} | Не можем поиграть в игру!")
                await asyncio.sleep(random.uniform(*config.DELAYS['ERROR_PLAY']))
                play_passes -= 1
                continue

            logger.info(f"Thread {self.thread} | {self.account} | Играем в игру! GameId: {game_id}")
            await asyncio.sleep(random.uniform(*config.DELAYS['GAME']))

            msg, points = await self.claim_game(game_id)
            if isinstance(msg, bool) and msg:
                logger.success(f"Thread {self.thread} | {self.account} | Закончили играть!; заработано: {points}")
            else:
                logger.error(f"Thread {self.thread} | {self.account} | Не можем поиграть в игру; {msg}")
                await asyncio.sleep(random.uniform(*config.DELAYS['ERROR_PLAY']))
    ...
    async def claim_daily_reward(self):
        ...
        if await resp.text() == 'OK':
            logger.success(f"Thread {self.thread} | {self.account} | Получена ежедневная награда!")
    ...
    async def login(self):
        ...
        if query is None:
            logger.error(f"Thread {self.thread} | {self.account} | Сессия {self.account} невалидна")
            await self.logout()
            return None
        ...
        while True:
            ...
            if resp.status == 520:
                logger.warning(f"Thread {self.thread} | {self.account} | Повторный вход в систему...")
                await asyncio.sleep(10)
                continue
            else:
                break
        ...
        if resp_json.get('justCreated'):
           logger.success(f"Thread {self.thread} | {self.account} | Зарегистрированный!")
            ...
            if not (await self.client.get_me()).username:
                while True:
                    ...
                    if await self.client.set_username(username):
                        logger.success(f"Thread {self.thread} | {self.account} | Установить имя пользователя @{username}")
                        break
